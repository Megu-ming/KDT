...
class FClass
{
	// 기본 접근 지정자: private
public:
	// 프로그램이 켜질 때 해당 인스턴스가 만들어진다
	// 꺼질 때 삭제된다.
	// static 변수에 inline키워드를 붙이면
	// header에 선언만 해도 인스턴스를 만들어 준다.
	// 하지만 dll에서 사용할 때 아주 주의해서 사용해야 한다.
	...
	// __thiscall의 특성을 생각하면
	// this pointer에 접근하지 않으면 문제가 없다.
	void Hello()
	{
		std::cout << "Hello\n";
	}
...
public:
	...
	FClass(FClass&& InOther) noexcept
		...
		// string은 특정 상황을 만족했을 때 데이터를 stack에 보관
		// 이 상황일 경우 복사가 move를 한다고 하더라도 발생
		// 이 경우에는 move가 의미가 없다
		// 하지만 문자열이 긴 경우 heap에 할당되는데
		// 이럴 때는 주소만 변경하면 되기 때문에 빠르다.
