...
int main()
{
	...
	}
	{
		...
		// 동적 할당된 데이터가 메모리 공간에 존재하고 이를 포인터 등으로 이어주고 힜다
		...
		// [Queue]: 선입선출(FIFO, First in first out), 먼저 들어온 것이 먼저 나온다)
		// #include <queue>
		...
		// pair: 두 데이터를 들고 있는 컨데이터
		{
			std::pair<std::string, std::string> Pair{ "Hello", "Player0" };
			std::pair<std::string, int> Pair2{ "Player100", 100 };
		...
		// 동일한 키에 대해서 여러 값을 보관하려면 multimap을 사용하면 됩니다.
		{
			...
			//Map.insert(std::make_pair(0, "Test2")); // 예는 무시. 이미 key0이 있다
			Map[0] = "Test2";
			Map[1] = "Test5";

			std::string Data = Map[999];
			std::map<int, std::string>::iterator It = Map.find(999); // 못찾으면 end()가 반환
			auto It2 = Map.find(576);
			if (It == Map.end())
			{
				std::cout << "못찾음\n";
			}
